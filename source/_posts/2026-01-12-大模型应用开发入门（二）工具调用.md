...
若想自行进行控制，需要某些情况下，将 `ToolCallingChatOptions` 的 `internalToolExecutionEnabled` 属性设为 `false`。该参数需要传入prompt的`chatOptions`
...
chatMemory.clear(sessionId);        chatMemory.add(sessionId,toolExecutionResult.conversationHistory());
```
